...
describe('InventoryService - Stock Level Methods', () => {
  ...
  describe('reserveStock', () => {
    it('should reserve stock successfully', async () => {
      ...
      inventoryService.logTransaction = jest.fn().mockResolvedValue({});

      const result = await inventoryService.reserveStock(validItemId, validWarehouseId, 50, {
        orderId: 'order123',
        userId: 'user123',
      });
      ...
      expect(mockInventory.reserve).toHaveBeenCalledWith(50);
    ...
    it('should throw error when insufficient stock available', async () => {
      ...
      await expect(
        inventoryService.reserveStock(validItemId, validWarehouseId, 50),
    ...
    it('should throw error when itemId is missing', async () => {
      await expect(
        inventoryService.reserveStock(null, validWarehouseId, 50),
      ).rejects.toThrow('Item ID is required');
    });
  });
  ...
  describe('releaseReservation', () => {
    it('should release reservation successfully', async () => {
      const mockInventory = {
        ...
        releaseReservation: jest.fn().mockReturnValue(true),
        save: jest.fn().mockResolvedValue(true),
      };
      ...
      const result = await inventoryService.releaseReservation(validItemId, validWarehouseId, 30, {
        orderId: 'order123',
      ...
      expect(result.success).toBe(true);
      expect(result.releasedQuantity).toBe(30);
      expect(mockInventory.releaseReservation).toHaveBeenCalledWith(30);
      expect(mockInventory.save).toHaveBeenCalled();
    });
    ...
    it('should throw error when trying to release more than reserved', async () => {
      const mockInventory = {
        quantity: 100,
        reservedQuantity: 20,
        releaseReservation: jest.fn().mockReturnValue(false),
      };

      Inventory.findOne = jest.fn().mockResolvedValue(mockInventory);

      await expect(
        inventoryService.releaseReservation(validItemId, validWarehouseId, 50),
  ...
  describe('getActiveReservations', () => {
    it('should return active reservations for an item', async () => {
      const mockReservations = [
        {
          item: {
            _id: validItemId, code: 'ITEM001', name: 'Test Item', unit: 'piece',
          },
      ...
      const result = await inventoryService.getActiveReservations(validItemId);
    ...
    it('should filter reservations by warehouse', async () => {
      const mockReservations = [
        {
          item: {
            _id: validItemId, code: 'ITEM001', name: 'Test Item', unit: 'piece',
      ...
      const result = await inventoryService.getActiveReservations(validItemId, { warehouseId: validWarehouseId });
    ...
    it('should throw error when itemId is missing', async () => {
      await expect(
        inventoryService.getActiveReservations(null),
      ).rejects.toThrow('Item ID is required');
    });
  });
});
