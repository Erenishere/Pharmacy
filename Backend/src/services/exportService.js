...
class ExportService {
  ...
  async exportInvoicesToExcel(filters = {}) {
    ...
    // Build query
    const query = { type: { $in: ['sales', 'return_sales'] } };
    ...
    // Define columns
    const columns = [
      { key: 'invoiceNumber', label: 'Invoice No', width: 15 },
      { key: 'invoiceDate', label: 'Date', width: 12 },
      { key: 'salesType', label: 'Type', width: 10 },
      { key: 'customerName', label: 'Customer', width: 25 },
      { key: 'salesmanName', label: 'Salesman', width: 20 },
      { key: 'grossTotal', label: 'Gross Total', width: 15 },
      { key: 'discountTotal', label: 'Discount', width: 12 },
      { key: 'gstTotal', label: 'GST', width: 12 },
      { key: 'netBillTotal', label: 'Net Total', width: 15 },
    ...
    const data = invoices.map((inv) => ({
      ...
      salesType: inv.salesType || 'new',
      customerName: inv.customerId?.name || inv.customerName || 'N/A',
      salesmanName: inv.salesmanId?.name || 'N/A',
      grossTotal: inv.totals?.grossTotal || 0,
      discountTotal: inv.totals?.discountTotal || 0,
      gstTotal: inv.totals?.gstTotal || 0,
      netBillTotal: inv.totals?.netBillTotal || inv.totals?.grandTotal || 0,
      status: inv.status || 'draft',
      paymentStatus: inv.paymentStatus || 'pending',
    }));

    return await this.exportToExcel(data, columns, 'Sales Invoices');
  }
   ...
   * @returns {Promise<Buffer>} PDF file buffer
   */
  async exportInvoicesToPDF(filters = {}) {
    const Invoice = require('../models/Invoice');
    ...
    const summary = {
      ...
      totalAmount: invoices.reduce((sum, inv) => sum + (inv.totals?.netBillTotal || inv.totals?.grandTotal || 0), 0),
      totalGST: invoices.reduce((sum, inv) => sum + (inv.totals?.gstTotal || 0), 0),
      totalDiscount: invoices.reduce((sum, inv) => sum + (inv.totals?.discountTotal || 0), 0),
    };

    // Define columns
    const columns = [
      { key: 'invoiceNumber', label: 'Invoice No', width: 80 },
      { key: 'invoiceDate', label: 'Date', width: 70 },
      { key: 'customerName', label: 'Customer', width: 120 },
      { key: 'netBillTotal', label: 'Amount', width: 80 },
    ...
    return await this.exportToPDF(
      ...
      { title: 'Sales Invoices Report', orientation: 'landscape' },
    );
  }

  /**
   * Export sales report to Excel
   * @param {Object} reportData - Report data with summary and details
   * @returns {Promise<Buffer>} Excel file buffer
   */
  async exportReportToExcel(reportData) {
    if (!reportData || !reportData.data) {
      throw new Error('Report data is required');
    }

    const columns = reportData.columns || this._getColumnsForReportType(reportData.reportType || 'sales');
    return await this.exportToExcel(reportData.data, columns, reportData.title || 'Sales Report');
  }
  ...
  async exportReportToPDF(reportData) {
    ...
    return await this.exportToPDF(
      { ...reportData, columns },
      {
        title: reportData.title || 'Sales Report',
