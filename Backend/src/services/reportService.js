...
class ReportService {
  ...
  async generateSalesReport(params) {
    const {
      startDate, endDate, customerId, salesmanId, groupBy = 'date',
    } = params;
  ...
  async generatePurchaseReport(params) {
    const {
      startDate, endDate, supplierId, groupBy = 'date',
    } = params;
  ...
  async getSchemeAnalysis(params) {
    const {
      startDate, endDate, customerId, supplierId, invoiceType = 'sales',
    } = params;
  ...
  async getSchemeInvoices(params) {
    const {
      schemeId, startDate, endDate, invoiceType = 'sales',
    } = params;
  ...
  async getDiscountBreakdown(params) {
    const {
      startDate, endDate, invoiceType = 'all', discountType = 'all', claimAccountId,
    } = params;
    ...
    ['discount1', 'discount2'].forEach((discType) => {
      Object.keys(breakdown[discType].byInvoiceType).forEach((invType) => {
        breakdown[discType].byInvoiceType[invType].amount = Math.round(breakdown[discType].byInvoiceType[invType].amount * 100) / 100;
      });
    });
  ...
  async getTaxReport(params) {
    const {
      startDate, endDate, invoiceType = 'all', customerId, supplierId,
    } = params;
  ...
  async getDiscountBreakdown(params) {
    const {
      startDate, endDate, discountType = 'all', claimAccountId,
    } = params;
  ...
  async getWarehouseMovementReport(warehouseId, startDate, endDate, options = {}) {
    ...
    return {
      ...
      movementsByItem: Object.values(movementsByItem).sort((a, b) => b.movementCount - a.movementCount),
  ...
  async getDiscountBreakdown(params) {
    ...
    const invoiceTypeBreakdown = {
      sales: {
        invoiceCount: 0, discount1Amount: 0, discount2Amount: 0, totalDiscountAmount: 0,
      },
      purchase: {
        invoiceCount: 0, discount1Amount: 0, discount2Amount: 0, totalDiscountAmount: 0,
      },
  ...
  async getWarehouseMovementReport(warehouseId, options = {}) {
    const {
      startDate, endDate, itemId, movementType,
    } = options;
