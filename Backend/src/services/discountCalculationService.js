 ...
 * Consolidated with logic from discountService.js
 */
class DiscountCalculationService {
  ...
  async validateClaimAccountForDiscount2(claimAccountId) {
    ...
    if (typeof account.canBeUsedForClaims === 'function') {
      ...
      // Fallback validation if method doesn't exist (e.g. older model version)
      if (!['adjustment', 'claim', 'expense'].includes(account.accountType)) {
        throw new Error(`Account '${account.name}' cannot be used for discount claims. Must be adjustment, claim, or expense type.`);
      }
    }
  ...
  async getAvailableClaimAccounts() {
    const Account = require('../models/Account');
    if (typeof Account.getClaimAccounts === 'function') {
  ...
  /**
   * Validate invoice discount configuration across all items
   * (Migrated from discountService.js)
  ...
  async validateInvoiceDiscounts(invoiceData) {
    ...
    const hasDiscount2 = items.some((item) => item.discount2Percent && item.discount2Percent > 0);
 ...
 */
  async calculateLineTotal(lineItem, claimAccountId) {
    const {
      quantity, unitPrice, discount1Percent = 0, discount2Percent = 0, taxRate = 0,
    } = lineItem;
    ...
    // Apply discounts
    // Using internal method rather than applyDiscounts for consistency
    const discounts = this.applySequentialDiscounts(lineSubtotal, discount1Percent, discount2Percent, claimAccountId);

    // Calculate tax on amount after discounts
    const taxAmount = (discounts.finalAmount * taxRate) / 100;
    const lineTotal = discounts.finalAmount + taxAmount;

    return {
      lineSubtotal,
      discount1Amount: discounts.discount1.amount,
      discount2Amount: discounts.discount2.amount,
      totalDiscount: discounts.totalDiscount.amount,
      taxableAmount: discounts.finalAmount,
      taxAmount,
      lineTotal,
    };
  }
