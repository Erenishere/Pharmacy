...
const stockMovementSchema = new mongoose.Schema({
  ...
  // Carton/Box/Unit quantity breakdown for transfers
  quantities: {
    qtyCtn: {
      type: Number,
      default: 0,
  ...
  status: {
    ...
    enum: {
      ...
      message: 'Status must be one of: pending, in_transit, completed, cancelled',
    },
    default: 'completed',
  },
  // Approval workflow fields for stock adjustments
  approvalStatus: {
    type: String,
    enum: {
      values: ['pending_approval', 'approved', 'rejected', 'auto_approved', 'not_required'],
      message: 'Approval status must be one of: pending_approval, approved, rejected, auto_approved, not_required',
    },
...
stockMovementSchema.index({ approvedBy: 1 });
stockMovementSchema.index({ referenceType: 1, approvalStatus: 1 });
stockMovementSchema.index({ warehouse: 1 });
stockMovementSchema.index({ 'transferInfo.fromWarehouse': 1 });
stockMovementSchema.index({ 'transferInfo.toWarehouse': 1 });
stockMovementSchema.index({ 'transferInfo.transferId': 1 });
...
// Instance method to check if movement can be cancelled
stockMovementSchema.methods.canBeCancelled = function () {
  return ['pending', 'in_transit'].includes(this.status);
};

// Instance method to check if movement is in transit
stockMovementSchema.methods.isInTransit = function () {
  return this.status === 'in_transit';
};

// Instance method to check if movement is completed
stockMovementSchema.methods.isCompleted = function () {
  return this.status === 'completed';
};

// Instance method to cancel movement
stockMovementSchema.methods.cancel = function () {
  if (!this.canBeCancelled()) {
    throw new Error('Cannot cancel a completed or already cancelled movement');
  }
  this.status = 'cancelled';
  return this.save();
};

// Instance method to mark as in transit
stockMovementSchema.methods.markInTransit = function () {
  if (this.status !== 'pending') {
    throw new Error('Only pending movements can be marked as in transit');
  }
  this.status = 'in_transit';
  return this.save();
};

// Instance method to complete movement
stockMovementSchema.methods.complete = function () {
  if (!['pending', 'in_transit'].includes(this.status)) {
    throw new Error('Only pending or in-transit movements can be completed');
  }
...
// Static method to find movements by status
stockMovementSchema.statics.findByStatus = function (status, options = {}) {
...
// Static method to find in-transit transfers
stockMovementSchema.statics.findInTransitTransfers = function () {
  return this.find({
    referenceType: { $in: ['transfer', 'warehouse_transfer'] },
    status: 'in_transit',
  })
    .sort({ movementDate: -1 })
    .populate('itemId', 'code name')
...
// Static method to find pending movements
stockMovementSchema.statics.findPendingMovements = function () {
...
stockMovementSchema.statics.calculateStockBalance = async function (itemId, warehouseId, asOfDate = new Date()) {
  const match = {
    ...
    status: { $in: ['completed'] }, // Only count completed movements
  };
...
stockMovementSchema.statics.getMovementHistory = function (itemId, warehouseId, options = {}) {
  const {
    startDate, endDate, limit = 100, page = 1,
  } = options;
  const skip = (page - 1) * limit;
...
stockMovementSchema.pre('save', function (next) {
  ...
  if (!this.status) {
    if (this.referenceType === 'warehouse_transfer' || this.referenceType === 'transfer') {
      this.status = 'pending';
    } else {
      this.status = 'completed';
    }
