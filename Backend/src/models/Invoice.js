...
const invoiceItemSchema = new mongoose.Schema({
  // Sales Management - Enhanced item fields for sales (Requirement 1.11-1.27)
  itemId: {
...
const invoiceSchema = new mongoose.Schema({
  ...
  // Sales Management - Sales type field (Requirement 1.2)
  salesType: {
    type: String,
    enum: ['new', 'return'],
    default: 'new',
  },
  // Invoice source: 'admin' = created via admin invoice form, 'pos' = created via POS terminal
  invoiceSource: {
    type: String,
    enum: ['admin', 'pos', 'import'],
    default: 'admin',
  },
  customerId: {
    type: mongoose.Schema.Types.ObjectId,
  ...
  // Sales Management - Other title field (Requirement 1.7)
  otherTitle: {
    type: String,
    trim: true,
    maxlength: [200, 'Other title cannot exceed 200 characters'],
  },
  poNumber: {
    type: String,
    trim: true,
  ...
  // Sales Management - Tax invoice type (Requirement 1.9)
  taxInvoiceType: {
    type: String,
    enum: ['normal', 'sales_tax'],
    default: 'normal',
  },
  // Sales Management - Advance tax rate (Requirement 1.6, 1.23)
  advanceTaxRate: {
    type: Number,
    enum: [0, 0.5, 2.5],
    default: 0,
  },
  // Sales Management - Detail note (Requirement 1.41)
  detailNote: {
    type: String,
    trim: true,
    maxlength: [1000, 'Detail note cannot exceed 1000 characters'],
  },
  warrantyInfo: {
    type: String,
    trim: true,
  ...
  // Purchase-specific: Quality Control (Requirement 8)
  qualityControlNotes: {
    type: String,
    trim: true,
    maxlength: [1000, 'Quality control notes cannot exceed 1000 characters'],
  },
  goodsReceiptNumber: {
    type: String,
    trim: true,
    maxlength: [100, 'Goods receipt number cannot exceed 100 characters'],
  },
  status: {
    type: String,
    enum: ['draft', 'confirmed', 'paid', 'cancelled', 'held'],
    default: 'draft',
  },
  paymentStatus: {
    type: String,
  ...
  heldReason: {
    type: String,
    trim: true,
    maxlength: [200, 'Held reason cannot exceed 200 characters'],
  },
  confirmedAt: {
    type: Date,
  },
  ...
  // Sales Management - Enhanced totals for sales (Requirement 1.26-1.33)
  totals: {
    subtotal: {
      type: Number,
    ...
    discountPercentOverall: {
      type: Number,
      default: 0,
      min: [0, 'Overall discount percent cannot be negative'],
      max: [100, 'Overall discount percent cannot exceed 100%'],
    },
    ...
    netBillTotal: {
      type: Number,
      default: 0,
      // min: [0, 'Net bill total cannot be negative'],
    },
  ...
  // Sales Management - Status workflow (Requirement 1.43, 1.44)
  status: {
    type: String,
...
invoiceSchema.index({ type: 1, salesType: 1 }); // Sales Management
...
invoiceSchema.index({ salesmanId: 1 }); // Sales Management
invoiceSchema.index({ salesmanId: 1, invoiceDate: -1 }); // Sales Management
invoiceSchema.index({ poId: 1 });
invoiceSchema.index({ expiryDate: 1 });
invoiceSchema.index({ 'items.batchNumber': 1 }); // Sales Management - batch tracking
