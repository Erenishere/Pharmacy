  ...
  try {
    ...
    const warehouses = [
      {
        ...
        location: {
          address: '123 Industrial Area', city: 'Karachi', state: 'Sindh', country: 'Pakistan', postalCode: '75700',
        },
        contact: { phone: '+92-21-1234567', email: 'main@pharma.com' },
        isActive: true,
        capacity: 10000,
      },
      {
        ...
        location: {
          address: '45 Sundar Estate', city: 'Lahore', state: 'Punjab', country: 'Pakistan', postalCode: '54000',
        },
        contact: { phone: '+92-42-9876543', email: 'lahore@pharma.com' },
        isActive: true,
        capacity: 8000,
      },
      {
        ...
        location: {
          address: '78 I-9 Industrial', city: 'Islamabad', state: 'ICT', country: 'Pakistan', postalCode: '44000',
        },
        contact: { phone: '+92-51-5555555', email: 'isb@pharma.com' },
        isActive: true,
        capacity: 5000,
      },
    ];
    ...
    for (const item of items) {
      for (const wh of createdWarehouses) {
        ...
        batches.push({
          batchNumber: `BTH260${String(idx++).padStart(4, '0')}`, item: item._id, warehouse: wh._id, supplier, manufacturingDate: addMonths(now, -6), expiryDate: addMonths(now, 18), quantity: 500, remainingQuantity: 450, unitCost: cost, totalCost: 500 * cost, status: 'active', notes: `Regular stock - ${item.name}`, referenceType: 'PURCHASE_ORDER', referenceNumber: `PO-${idx}`,
        });
        batches.push({
          batchNumber: `BTH260${String(idx++).padStart(4, '0')}`, item: item._id, warehouse: wh._id, supplier, manufacturingDate: addMonths(now, -3), expiryDate: addMonths(now, 24), quantity: 300, remainingQuantity: 300, unitCost: cost, totalCost: 300 * cost, status: 'active', notes: `Fresh stock - ${item.name}`, referenceType: 'PURCHASE_ORDER', referenceNumber: `PO-${idx}`,
        });
        batches.push({
          batchNumber: `EXP260${String(idx++).padStart(4, '0')}`, item: item._id, warehouse: wh._id, supplier, manufacturingDate: addMonths(now, -12), expiryDate: addDays(now, 15), quantity: 100, remainingQuantity: 75, unitCost: cost, totalCost: 100 * cost, status: 'active', notes: `EXPIRING SOON - ${item.name}`, referenceType: 'PURCHASE_ORDER', referenceNumber: `PO-${idx}`,
        });
        batches.push({
          batchNumber: `EXP260${String(idx++).padStart(4, '0')}`, item: item._id, warehouse: wh._id, supplier, manufacturingDate: addMonths(now, -18), expiryDate: addDays(now, -10), quantity: 50, remainingQuantity: 30, unitCost: cost, totalCost: 50 * cost, status: 'expired', notes: 'EXPIRED - needs disposal', referenceType: 'PURCHASE_ORDER', referenceNumber: `PO-${idx}`,
        });
        batches.push({
          batchNumber: `DPL260${String(idx++).padStart(4, '0')}`, item: item._id, warehouse: wh._id, supplier, manufacturingDate: addMonths(now, -9), expiryDate: addMonths(now, 12), quantity: 200, remainingQuantity: 0, unitCost: cost, totalCost: 200 * cost, status: 'depleted', notes: 'Fully consumed', referenceType: 'PURCHASE_ORDER', referenceNumber: `PO-${idx}`,
        });
        batches.push({
          batchNumber: `QRT260${String(idx++).padStart(4, '0')}`, item: item._id, warehouse: wh._id, supplier, manufacturingDate: addMonths(now, -2), expiryDate: addMonths(now, 22), quantity: 100, remainingQuantity: 100, unitCost: cost, totalCost: 100 * cost, status: 'quarantined', notes: 'Quality check pending', referenceType: 'OTHER', referenceNumber: `QC-${idx}`,
        });
      }
    }
    ...
    const stats = {
      active: 0, expired: 0, depleted: 0, quarantined: 0,
    };
