...
const CUSTOMER_TYPE_FILTER_VALUES = ['retail', 'wholesale', 'distributor', 'regular', 'customer', 'supplier', 'both'];
...
// Import/Export routes (must come first)
/**
 * @route   GET /api/customers/template
 * @desc    Download Excel template for accounts import
 * @access  Private
 */
router.get('/template', authenticate, customerController.downloadAccountsTemplate);

/**
 * @route   POST /api/customers/bulk-import
 * @desc    Bulk import accounts from Excel file
 * @access  Private (Admin, Data Entry)
 */
router.post(
  '/bulk-import',
  authenticate,
  requireRoles(['admin', 'data_entry', 'accountant']),
  upload.single('file'),
  customerController.bulkImportAccounts,
);

/**
 * @route   GET /api/customers/export
 * @desc    Export accounts to Excel or PDF
 * @access  Private
 */
router.get('/export', authenticate, customerController.exportAccounts);
...
router.get(
  ...
  [
    param('type')
      .isIn(CUSTOMER_TYPE_FILTER_VALUES)
      .withMessage(`Invalid type. Must be one of: ${CUSTOMER_TYPE_FILTER_VALUES.join(', ')}`),
    validate,
  ],
