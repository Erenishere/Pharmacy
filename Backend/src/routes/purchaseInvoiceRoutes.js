...
/**
 * @route   POST /api/invoices/purchase/return
 * @desc    Create a purchase return invoice
...
router.post(
  ...
  [
    body('originalInvoiceId')
      ...
      .custom(isValidObjectId)
      .withMessage('Invalid original invoice ID format'),
    body('returnItems')
      .isArray({ min: 1 })
      .withMessage('At least one return item is required'),
    body('returnItems.*.itemId')
      .notEmpty()
      .withMessage('Item ID is required for all return items')
      .custom(isValidObjectId)
      .withMessage('Invalid item ID format'),
    ...
    body('returnNotes')
      ...
      .trim()
      .isLength({ max: 1000 })
      .withMessage('Return notes cannot exceed 1000 characters'),
    validate,
  ],
  purchaseInvoiceController.createPurchaseReturn,
);

/**
 * @route   GET /api/invoices/purchase/:id/returnable
 * @desc    Get returnable items for a purchase invoice
 * @access  Private
 */
router.get(
  '/:id/returnable',
  ...
  purchaseInvoiceController.getReturnableItems,
);

/**
 * @route   POST /api/invoices/purchase/:id/validate-return
 * @desc    Validate return quantities for a purchase invoice
 * @access  Private
 */
router.post(
  '/:id/validate-return',
  ...
  purchaseInvoiceController.validateReturn,
);
