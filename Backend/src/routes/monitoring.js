...
/**
 * @route   GET /health/stock
 * @desc    Get stock data integrity health check
 * @access  Private (Admin only)
 */
router.get('/health/stock', authenticate, requireAdmin, async (req, res) => {
  try {
    const stockHealth = await stockReconciliationService.getStockHealthSummary();

    const statusCode = stockHealth.status === 'healthy' ? 200
      : stockHealth.status === 'warning' ? 200 : 503;
