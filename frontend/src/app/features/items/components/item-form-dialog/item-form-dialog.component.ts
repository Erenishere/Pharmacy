...
export class ItemFormDialogComponent implements OnInit {
    ...
    createForm(): FormGroup {
        return this.fb.group({
            ...
            // Supplier
            primarySupplierId: [''],
            supplierItemCode: ['', Validators.maxLength(50)],
            leadTimeFromSupplier: [0, Validators.min(0)],
            ...
            handlingInstructions: ['', Validators.maxLength(500)],
    ...
    populateForm(item: Item): void {
        this.itemForm.patchValue({
            ...
            primarySupplierId: item.supplier?.primarySupplierId || '',
            supplierItemCode: item.supplier?.supplierItemCode || '',
            leadTimeFromSupplier: item.supplier?.leadTimeFromSupplier || 0,
            
            taxRegistrationNumber: item.regulatory?.taxRegistrationNumber || '',
            complianceStatus: item.regulatory?.complianceStatus || 'Compliant',
    ...
    onSubmit(): void {
        ...
        const itemData = {
            ...
            supplier: {
                primarySupplierId: formValue.primarySupplierId,
                supplierItemCode: formValue.supplierItemCode,
                leadTimeFromSupplier: formValue.leadTimeFromSupplier
            },
            regulatory: {
                taxRegistrationNumber: formValue.taxRegistrationNumber,
                complianceStatus: formValue.complianceStatus
            },
            metadata: {
        ...
        if (this.isEditMode) {
            this.itemService.updateItem(this.data.item!._id, itemData).subscribe({
                next: (response) => {
                    if (response.success) {
