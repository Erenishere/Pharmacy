...
export class InvoiceService {
  ...
  createPurchaseReturn(returnData: any): Observable<ApiResponse<Invoice>> {
    return this.http.post<ApiResponse<Invoice>>(`${this.purchaseUrl}/${returnData.originalInvoiceId}/return`, returnData);
  }

  // ─── Sales Returns ─────────────────────────────────────────────
  private salesReturnUrl = `${environment.apiUrl}/sales-returns`;

  createSalesReturn(data: any): Observable<ApiResponse<Invoice>> {
    return this.http.post<ApiResponse<Invoice>>(this.salesReturnUrl, data);
  }

  processReturn(id: string): Observable<ApiResponse<Invoice>> {
    return this.http.patch<ApiResponse<Invoice>>(`${this.salesReturnUrl}/${id}/process`, {});
  }

  getSalesReturns(params: any = {}): Observable<ApiResponse<Invoice[]>> {
    let httpParams = new HttpParams();
    Object.entries(params).forEach(([key, value]) => {
      if (value !== undefined && value !== null && value !== '') {
    ...
    return this.http.get<ApiResponse<Invoice[]>>(this.salesReturnUrl, { params: httpParams });
  }

  getSalesReturnById(id: string): Observable<ApiResponse<Invoice>> {
    return this.http.get<ApiResponse<Invoice>>(`${this.salesReturnUrl}/${id}`);
  }

  getReturnCreditNote(id: string): Observable<ApiResponse<any>> {
    return this.http.get<ApiResponse<any>>(`${this.salesReturnUrl}/${id}/credit-note`);
  }

  getInvoices(params: any = {}): Observable<ApiResponse<Invoice[]>> {
    if (params.type === 'purchase') {
      return this.getPurchaseInvoices(params);
    } else {
      return this.getSalesInvoices(params);
    }
  }

  getInvoiceById(id: string, type: 'sales' | 'purchase' = 'purchase'): Observable<ApiResponse<Invoice>> {
    if (type === 'purchase') {
      return this.getPurchaseInvoiceById(id);
    } else {
      return this.getSalesInvoiceById(id);
    }
  }
}
